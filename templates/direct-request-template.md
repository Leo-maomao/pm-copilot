...
If enough information is available, create `prd.md` and the matching UI review deliverable in my request language, including localized headings, labels, statuses, UI notes, and numbered annotations. When the request is a structured reference or document handoff and I say no PRD is needed, use `catalog.md`/`reference.md` and optional document prototype HTML as the primary delivery instead. When frontend source exists, inspect it as read-only evidence. Use `prototype-<platform>.html` or a structured UI specification as a review artifact; do not modify the current repository, implement the UI, or claim 1:1 product parity for proposed behavior.
