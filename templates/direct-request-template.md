...
If enough information is available, create `prd.md` and the matching UI deliverable in my request language, including localized headings, labels, statuses, UI notes, and numbered annotations. When the request is a structured reference or document handoff and I say no PRD is needed, use `catalog.md`/`reference.md` and optional document prototype HTML as the primary delivery instead. When frontend source exists, use source-backed preview/delta files. If I ask to implement the UI in the current repository first and then hand off a 1:1 artifact, extract the finished running UI into source-derived HTML with editable annotations. Use `prototype-<platform>.html` only for source-extracted HTML, no-source, explicit portable HTML without source implementation, explicit redesign/greenfield, concrete source-rendering blockers, or document prototype review.
