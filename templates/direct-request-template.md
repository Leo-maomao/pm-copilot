...
If enough information is available, create `prd.md` and the matching UI deliverable in my request language, including localized headings, labels, statuses, UI notes, and numbered annotations. When frontend source exists, use source-backed preview/delta files; use `prototype-<platform>.html` only for no-source, explicit portable HTML, explicit redesign/greenfield, or concrete source-rendering blockers.
