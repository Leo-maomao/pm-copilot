...
9. For repo-backed prototype-only UI deliveries, verify `isolated_ui_prototype` records a read-only host mutation policy, target surface, `baseline_layer`, `delta_layer`, non-empty source-to-demo mapping, backend simulation method, parity claim, and limitations. Verify marker clicks open local popovers beside the marked component, not full-screen/global note modals. Treat missing isolation evidence, shallow style evidence, or unexpected production file edits as a High finding unless the user explicitly requested implementation.
