...
description: Use when generating annotated UI deliverables for Web, H5, App, Mini Program, source previews, extracted HTML, or portable HTML.
---
...
2. Load `artifacts/ui-delivery-contract.md` when available.
...
6. For repo-backed UI-delivery work, define the isolation boundary and artifact mode before writing UI. Host production flows are read-only by default, but host frontend source presence authorizes isolated preview-only source files. Use `source_delta_patch` by default whenever host frontend source and a preview surface exist: import/render the original host page/screen/components/styles/assets as baseline, then add the requested feature only in preview-only delta files. Platform-specific variants are `code_preview_route`, `storybook_or_demo`, `mini_program_preview`, and `app_preview_screen`. Use `source_extract_html` when the requested or inferred handoff path is: PM generates or adjusts the target UI inside the original project first, then extracts that finished region as an independent annotated HTML file for engineering. If the user explicitly asks to implement the feature in the current repo first, record that user-approved source-change scope and extract from the running implementation. Use `portable_html` only when the user's raw request explicitly asks for a portable/standalone/HTML artifact without source implementation, explicitly asks to redesign/rebuild/from-scratch/stop reusing the original UI, no host source exists, or source rendering was attempted and blocked by a concrete command, browser, simulator, dependency, or preview-surface failure. "Only generate a prototype" means review scope only; it does not authorize portable HTML or greenfield UI. Production files being read-only is not a blocking reason because isolated preview files are allowed. When `portable_html` cannot inherit a rendered source baseline, capture available visual evidence and mark the parity claim as fidelity-limited.
7. For source-backed previews, record the preview command, route/story/screen, and changed preview/delta or user-approved implementation files; do not hand off only a localhost URL. For `source_extract_html`, generate `prototype-<platform>.html` or offline `index.html` after the host-source preview or implementation renders, and record the extraction selector, source preview target, source-region screenshot, extracted-region screenshot, region diff result, interaction replay scope and results, extraction command, style capture method, asset handling, editable annotation layer/configuration, source-change scope, and limitation. For `portable_html`, generate `prototype-<platform>.html`; `index.html` is allowed as the offline folder entry only when the user explicitly asks for a portable/offline handoff.
...
- UI deliverable: source-rendered preview/delta files, source-extracted HTML handoff, or portable HTML artifact
