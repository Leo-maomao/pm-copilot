...
7. Treat a screenshot as evidence, not decoration. When a requirement needs one, use the first viable source method: Playwright for a local or hosted preview, Chrome DevTools for an existing authenticated browser surface, or Computer Use for a non-automatable surface. Save the real image in `assets/`, then link it inline. Crop to the functional target plus locating context and necessary comparison context; reject both irrelevant full-screen captures and unidentifiable micro-crops. Do not use model-generated images or screenshots from another requirement. If the requirement genuinely needs a figure but every trusted capture path fails, use the controlled `占位图：<name>.png<br><small>位置：...；用途：...</small>` fallback inline, and record the failed paths in `run-log.yaml`. If a figure does not improve reviewability, omit the `图示` row.
8. Add `需求调研`, `多语言需求`, and `埋点需求` only when they have real, decision-relevant content. For every research row, state the source fact and the PRD decision it changes. For every multilingual row, provide only user-visible copy that actually changes. A tracking table uses only `名称`、`标识`、`时机`、`参数`、`备注`: select access events, meaningful clicks/operations, important outcomes, and value signals such as duration, depth, exposure, completion, or retention when the user journey needs them. Build events from a measurable user journey rather than listing every control. `参数` contains additional context only; for example, project creation may add 创建人标识、创建时间、项目名称, while canvas creation may add 创建人标识、创建时间、关联项目名称. Leave `参数` blank when no extra property is required; never fill it with “无”“不涉及”“-” or a restatement of the event. Omit empty optional sections and explanatory labels.
