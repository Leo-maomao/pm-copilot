...
| Existing surface fit | Invents unrelated product UI, mutates host files, or presents proposed behavior as implemented | Some existing context reflected | Grounds the review artifact in read-only source or visual evidence, separates observed and proposed behavior, and names fidelity limits and the implementation owner |
