...
The repository `VERSION` is the release source of truth for both the global runtime and the Codex plugin manifest. A normal installation links the personal marketplace plugin source to the repository plugin, preserving any old source as a timestamped backup, then reinstalls local `pm-copilot@personal` when the Codex CLI is available. This refreshes its cached UI version. Use `--skip-plugin-refresh` only when that local plugin refresh is intentionally deferred.
