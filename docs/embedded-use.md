...
When PM Copilot sits inside a host repository, start the controller from its own checkout. Generated PRD artifacts stay under the current project's `pm-copilot-outputs` directory; no host code, configuration, or deployment state is changed.
