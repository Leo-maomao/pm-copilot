...
- For repo-backed UI prototypes, read the real host frontend code, component library, styles, and assets, keep host production files read-only by default, and generate an isolated HTML demo that mirrors the current surface with only the requested feature added.
- For repo-backed high-fidelity prototypes, use a host-rendered preview route or Storybook/demo when the user expects exact icons, real component-library behavior, or source-level fidelity; otherwise clearly label standalone HTML as an approximation.
- For repo-backed UI prototypes, separate the restored original UI as `baseline_layer` from the new feature as `delta_layer`; only the delta layer should carry visible markers, explanation dialogs, backend notes, tracking notes, and edge-case annotations.
- For UI prototypes, use visible red component markers, click-open/click-again-close local annotation popovers beside each marker, and current-state annotation lists instead of shrinking the product UI into a permanent side annotation board or opening full-screen marker notes.
