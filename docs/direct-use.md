...
- Source-backed UI preview handoff must include the changed preview/delta files, route/screen/story, and run command. Do not hand off only a localhost URL. If direct HTML is explicitly requested, generate compatibility HTML only when source-level parity can be limited or source rendering is not required.
