...
class PrdDeliveryEngineTest(unittest.TestCase):
    ...
    def assert_v2_template(self, folder: Path) -> None:
        markdown = (folder / "prd.md").read_text(encoding="utf-8")
        for value in (
            "## 一、文档说明", "## 二、需求背景", "## 四、需求清单", "## 五、需求详情",
            "| 需求来源 |", "| 目标用户 |", "| 影响范围 |", "| 文档状态 |",
            "| 文档负责人 |", "| 版本 | 日期 | 变更内容 | 负责人 |",
            "| 详情编号 | 需求名称 | 目标用户 | 用户场景 / 触发 |",
            "| 用户与场景 |", "| 需求入口 |", "| 需求详情 |", "| 设计与交互 |",
        ):
            self.assertIn(value, markdown)
        self.assertNotIn("YYYY-MM-DD", markdown)
        self.assertNotIn("### 5.1 需求名称", markdown)

    def test_new_implemented_composition_and_revision_deliver_in_one_confirmation(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self.deliver(root / "new", "--request", "为审批人增加审批提醒功能")
            self.assert_v2_template(root / "new")

            evidence = root / "evidence.json"
            evidence.write_text('{"observed": "导出功能"}', encoding="utf-8")
            self.deliver(root / "implemented", "--request", "已实现导出功能，生成 PRD", "--implemented-evidence", str(evidence))
            self.assert_v2_template(root / "implemented")

            source = root / "source.md"
            source.write_text("# 旧 PRD\n\n### 5.1 结算流程\n", encoding="utf-8")
            self.deliver(root / "composition", "--request", "组合结算流程生成 PRD", "--extract-from", str(source))
            self.assert_v2_template(root / "composition")
            ...
            self.assert_v2_template(revision)

    def test_revision_rejects_legacy_figure_rows(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            folder = Path(temporary) / "revision"
            self.deliver(folder, "--request", "生成审批提醒功能 PRD")
            baseline = (folder / "prd.md").read_text(encoding="utf-8")
            folder.joinpath("prd.md").write_text(
                baseline.replace("| 设计与交互 |", "| 图示 | ./assets/old.png |\n| 设计与交互 |", 1),
                encoding="utf-8",
            )
            result = self.run_controller(
                "--request", "更新审批提醒的失败反馈", "--run-folder", str(folder),
                "--revise", "--revision-requirement-id", "5.1",
            )
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("仅支持最新版 v2 PRD 基线", result.stdout)

    def test_missing_product_context_asks_once_before_confirmation(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            folder = Path(temporary) / "needs-input"
            result = self.run_controller("--request", "生成", "--run-folder", str(folder))
    ...
    def test_revision_reexecutes_completed_run_and_reuses_matching_figure_asset(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            folder = Path(temporary) / "revision"
            self.deliver(folder, "--request", "生成“字幕擦除”功能 PRD")
    ...
    def test_path_only_revision_requests_one_scope_question(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            folder = Path(temporary) / "revision"
            self.deliver(folder, "--request", "生成字幕擦除功能 PRD")
            result = self.run_controller(
    ...
    def test_assets_selectors_and_append_share_the_same_transaction(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.deliver(folder, "--request", "组合入口和结果生成 PRD", "--extract-from", str(first), "--extract-from", str(second), "--extract-selector", "5.1", "--extract-selector", "5.2", "--asset", str(asset))
