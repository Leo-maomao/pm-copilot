...
10. Choose prototype platform:
   ...
   - Use visible numbered annotation markers on the UI element being explained. The default UI marker is a small red circular badge using `annotation-marker`, `data-annotation-id`, and `data-annotation-placement="top-right"`; place it at a safe top-right position that is not clipped by overflow and does not force the component text to wrap. Clicking a marker opens an `annotation-dialog`, and a draggable top-right `annotation-toggle` with `data-draggable="true"` opens the current page/state `annotation-list`.
