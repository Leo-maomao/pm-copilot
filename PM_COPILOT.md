...
- Use manual capture only after automation recovery fails. If a screenshot is still unavailable after automated capture, tool setup or repair, authentication recovery, and manual fallback, omit the optional `图示` row unless that requirement genuinely needs visual evidence. In that case, use only the controlled inline `占位图：功能-状态.png` placeholder value, and record failed paths, reason, and replacement instruction in the run trace.
