...
`prd.html` is a document rendering of the PRD, not a UI prototype. It should use a fixed PM Copilot document shell with a left table of contents that starts from numbered sections, excludes the H1 title, uses stable ASCII anchors, syncs with reading position, and keeps a single content area. Avoid decorative cards, module blocks, unusual background colors, gradients, or nested scroll containers. Tables must preserve all columns, wrap content readably, and use consistent left alignment in Markdown and HTML; field/value tables should keep the field column readable without squeezing the value column. Mermaid diagrams must render as diagrams through local assets, not CDN. Images or image placeholders must appear inline exactly where reviewers need them, including inside requirement/detail tables when that is the relevant position; do not move them into a separate image list. When a requirement detail is written as a field/value table, the `图示`/`截图` row must contain the real image or missing-image marker in the same row and same value cell; do not place a blockquote placeholder or image paragraph immediately below the table. Real images should use local relative paths and support click-to-fullscreen or equivalent lightbox viewing. If the user will replace images manually, insert explicit inline placeholders at the intended location.

For implemented-feature PRDs, use `templates/implemented-feature-prd-template.md` as the default structure and render browser-readable delivery with `scripts/render_prd_html.py`. The PRD title must be one concise requirement sentence plus the requirement date, such as `# 优化团队权限设置体验 - 2026-06-29`, rather than a loose topic-list title plus `PRD`. The top-level section order is fixed: `文档信息`, `版本记录`, `需求背景`, `需求目标`, `需求调研`, `需求列表`, `需求详情`, `埋点需求`, `多语言需求`, `验收标准`, `测试建议`, and for implemented code also `代码实现说明`, `代码位置`, `验证结果`. Planned/non-implemented PRDs must omit those code-related sections. Use numbered headings such as `## 1. 文档信息`. If a required top-level section has no applicable content, keep it with a clear `不涉及：原因`; optional blocks and conditional sections with no content must be hidden. Frontend page/component or visual-state changes must include UI specifications inside the affected requirement detail, covering component/surface, layout/alignment, dimensions, spacing, typography, colors/tokens, icons/images, states, responsive behavior, accessibility/focus behavior when relevant, and visual acceptance notes. Flow diagrams are optional and belong inside the specific requirement detail they explain, not as fixed global `用户流程图`/`功能流程图` sections. Missing screenshots must be marked only with the exact inline `占位图` block at the relevant requirement position; do not call them `待补真实图`. In prose or subsection bodies, use the two-line blockquote marker; inside Markdown table cells, use the single-cell form `占位图：<file>.png<br>用途：<purpose>`. Recommend screenshot file names by content; when one object has multiple states, include the concrete state, for example `资料卡片-加载中.png` and `资料卡片-加载失败.png`, not `资料卡片-状态.png`. Screenshot coverage is by independent page, window, panel, or dialog; do not inflate the PRD with micro-state screenshots when one complete window screenshot covers them.
...
2. Match the user's language:
   ...
   - In Chinese PRDs, the `多语言需求` pure-text extraction block must default to Chinese-only user-facing copy. Do not list English/Chinese copy pairs unless the user explicitly asks for bilingual output; if English source strings were found in code, mention them as localization source notes outside the pure-text block.
...
8. After the clarification gate passes, create or update the product-manager delivery artifacts:
   ...
   - For `implemented-feature-prd` mode, do not create a standalone image, figure, or screenshot list by default. Images and missing-image markers must travel with the requirement, flow step, table row, dialog, or state they explain. If a requirement uses a two-column field/value table, place `![...](./assets/...)` or `占位图：...<br>用途：...` in the `图示`/`截图` row value cell; a table followed by a blockquote or image paragraph is invalid because reviewers lose the row-level association.
   ...
   - For `implemented-feature-prd` mode, cover every independent changed page, window, panel, or dialog, but avoid splitting micro-states when one screenshot captures the complete window or panel.
   ...
   - For copy/i18n sections, separate pure copy extraction from key mapping: the pure-text block contains only user-facing copy lines in the current delivery language, while i18n keys, usage positions, and notes belong in a table below it. Do not write `key = copy` lines inside the pure-text block, and do not include bilingual copy pairs unless the user requested bilingual output.
...
11. Run tool preflight and validation after file changes when possible:
   ...
   - When `prd.html` exists, validate that image paths resolve, images/placeholders are inline at the relevant PRD position, there is no separate screenshot list, requirement-detail tables keep all semantic columns visible, Mermaid diagrams are renderable, the TOC uses the fixed PM Copilot shell with stable ASCII anchors and includes numbered `h2`/`h3`/`h4` sections, tables are left-aligned and readable, and the page reads as a normal document instead of a decorative UI prototype.
...
11a. Practice-driven self-iteration after real usage:
   - When the user asks to optimize PM Copilot after a real delivery, treat the completed run as a pressure case. Load the run artifacts, user corrections, delivery-check output, and affected workflow files before editing generic PM Copilot sources.
   - Convert every observed issue into a reusable failure statement. Keep host-specific product names, local paths, routes, API fields, and UI vocabulary out of generic prompts, workflows, skills, contracts, and templates unless they appear only in fixture-scoped evals.
