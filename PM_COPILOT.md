...
10. Choose prototype platform:
   ...
   - Before delivery, verify prototype JavaScript parses, all primary buttons visibly change state, annotation markers open dialogs, the annotation toggle opens and closes the right-side current-state list panel, every marker dialog and page annotation panel number badge uses matching plain digit text, state switches stay fixed, and compact labels such as tabs or segmented controls do not fold because of annotation placement.
