...
10. Choose UI delivery platform and artifact mode:
   ...
   - Vocabulary rule: user words such as "prototype", "原型", "demo", "draft", or "only generate a prototype" describe review scope, not implementation method. PM Copilot's default UI deliverable is source-backed whenever source exists. Standalone HTML is a compatibility artifact mode, not the definition of UI delivery.
   ...
   - If the UI deliverable needs page/state switching controls, keep them in a stable fixed position outside the product layout, not embedded as abrupt content in the host surface.
   - Before delivery, verify standalone UI HTML JavaScript parses when HTML is generated, all primary buttons visibly change state, annotation markers open dialogs, the annotation toggle opens and closes the right-side current-state list panel, every marker dialog and page annotation panel number badge uses matching plain digit text plus the same rendered size, font sizing, and centered alignment as the UI marker, state switches stay fixed, and compact labels such as tabs or segmented controls do not fold because of annotation placement.
