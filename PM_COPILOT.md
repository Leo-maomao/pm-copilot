...
11. Run tool preflight and validation after file changes when possible:
   ...
   - When `prd.html` exists, validate that image paths resolve, images/placeholders are inline at the relevant PRD position, there is no separate screenshot list, requirement-detail tables keep all semantic columns visible, Mermaid diagrams are renderable, the TOC uses the fixed PM Copilot shell with stable ASCII anchors, tables are left-aligned and readable, and the page reads as a normal document instead of a decorative UI prototype.
