...
截图按内容命名；同一个对象有多个状态时使用“对象-具体状态”，例如 `文件上传-上传中.png`、`文件上传-上传失败.png`，不要使用 `文件上传-状态.png`。截图覆盖粒度按独立页面、窗口、面板或弹窗判断，不要把一个窗口里能同时看见的微状态拆成多张。生成或更新 HTML 需求文档时使用 `scripts/render_prd_html.py`，嵌入宿主项目时输出路径应是 `pm-copilot/outputs/<run-id>/`。
