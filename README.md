...
PM Copilot 是一个开源、平台中立的产品经理 Agent Workflow Kit。它帮助产品经理把模糊的产品需求转化为两个可交接成果：完整 PRD 和源码优先的带标注 UI 交付物。
...
- 适用于 Web、H5、App 或小程序场景的带标注 UI 交付物；有前端源码时默认用源码预览/增量补丁，只有无源码、明确便携 HTML、明确重做视觉或源码渲染被阻塞时才使用本地 HTML 兼容产物
...
2. 让 Agent 读取 `PM_COPILOT.md`，然后自然描述你的产品经理需求，例如：`我需要一份会员自动续费优化的 PRD、埋点方案和 H5 UI 交付物。`
...
| Web UI 交付物 | 有前端源码时使用源码驱动的预览路由、Storybook/demo 或 `source_delta_patch`，复用现有后台壳层、组件库和表格密度；仅在明确便携 HTML 或源码渲染被阻塞时才退回 `prototype-web.html` |
...
这个 Demo 重点展示：`repo-backed` 上下文读取、外部调研和仓库上下文分离、中文 PRD、现有 UI 源码增量交付、红色组件标注、研发交接、权限和边界状态覆盖。
...
生成兼容 HTML UI 交付物时，PM Copilot 应运行 `python3 scripts/validate_prototype_visual.py outputs/<run-id>`。生成源码驱动 UI 预览时，应通过宿主项目的 dev/preview/Storybook/模拟器路径运行并记录等价截图或浏览器证据。如果缺少 Playwright 或浏览器工具，应先运行或引导 `python3 scripts/setup_visual_validation.py`；只有安装失败、环境禁止启动浏览器，或用户拒绝安装时，才允许记录跳过原因。最终交付前应优先运行 `python3 scripts/run_delivery_checks.py outputs/<run-id> --language zh`，并把工具证据写入 `outputs/<run-id>/tool-results/`。当用户请求研发交接或上线就绪检查时，同一个运行目录也可以包含 `dev-tasks.yaml` 和 `launch-decision.yaml`。
...
| UI 交付和 UI 证据 | `multi-platform-prototype`、`design-system-audit` |
...
如果本次交付依赖外部调研或来源校验，请使用 `python3 scripts/preflight_tools.py --check-network <url> --require-network --strict` 做网络能力预检。`validate_prototype_visual.py` 在未指定 `--prototype` 时会校验运行目录中的全部受支持兼容 HTML 文件。
