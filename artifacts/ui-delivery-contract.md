...
UI deliverables are annotated artifacts used for product review, UI reference, and engineering handoff. In repo-backed products, the default deliverable is source-backed: import or render the original frontend code as the baseline, then add only the requested change through an isolated preview/delta layer. When product managers need to give engineering an independent HTML file after first shaping the UI in the original project, use `source_extract_html`: render the target UI in the host source preview, extract the selected region, and add the annotation handoff layer. Use `portable_html` for no-source work, explicit portable HTML requests without source implementation, explicit redesign/greenfield requests, or concrete source-rendering blockers.

Portable UI review files use `prototype-<platform>.html`; visual checks use `validate_prototype_visual.py`; runtime evidence lives in `ui_delivery_trace`. These are stable delivery interfaces. The word "prototype" does not permit a fake product shell or lower the fidelity and interaction requirements.
...
When the user supplies a screenshot, generated concept image, mockup, or cropped UI image as the target, the UI deliverable must run Image Reference Reconstruction Mode from `skills/multi-platform-ui-delivery/SKILL.md`. Record the reference source, exact dimensions, intended viewport, role of the image, visual inventory, asset decisions, screenshot comparison method, mismatches fixed, and remaining fidelity limits. The latest supplied reference image is the visual source of truth for the requested surface or delta, while repo-backed source-first baseline rules still apply when host frontend source exists.
...
The expected repo-backed artifact mode depends first on source availability. If host frontend source and a preview surface exist, use `source_delta_patch` by default: import/render the real frontend components, styles, assets, and platform chrome as the baseline, then add the new requirement only in isolated preview delta files. This does not require the user to say exact UI parity. Platform-specific source-rendered modes are `code_preview_route`, `storybook_or_demo`, `mini_program_preview`, and `app_preview_screen`. Use `source_extract_html` when a standalone engineering handoff is needed after the UI was first rendered in the host project. This mode is allowed for renderable repo-backed products because the HTML is derived from the source preview or user-approved host implementation, not hand-recreated. If the user explicitly asks to build the feature in the current repository first and then hand off a 1:1 artifact, record the changed implementation files, run the host UI, and extract the finished region from that running UI. Use `portable_html` only when the user's raw request explicitly asks for portable/standalone/HTML review without source implementation, explicitly asks to redesign/rebuild/from-scratch/stop reusing the original UI, no host source exists, or when source rendering was attempted and blocked by concrete command, browser, simulator, dependency, or preview-surface evidence; "only generate a prototype" means review scope only and is not a portable HTML or greenfield request. When `portable_html` cannot inherit a rendered source baseline, mark fidelity as limited. Backend-dependent behavior should be represented with coherent mock data, loading/empty/error/permission states, and annotations that name the expected data contract or API behavior when known.

A source-backed preview may be opened through a local dev URL, but the deliverable is not just that URL. Record the command, route/story/screen, changed preview/delta files, validation evidence, and any setup limits. If the user asks for direct HTML and exact source parity is not required, generate `portable_html`; if exact parity depends on host components, explain that the portable artifact is fidelity-limited.
...
- Source-change scope: whether the source region came from preview-only files, production-oriented implementation files, or another user-approved branch/worktree state. Include changed file paths when they are part of the user-approved implementation.
...
- Generated annotations should come from one editable annotation map or configuration block. Users should be able to add, remove, reorder, or edit the default notes by changing that config and the marker IDs, without rewriting copied product DOM or extracted source layout.
...
- The default callout shape is a small red circular badge using `annotation-marker`, `data-annotation-id`, and `data-annotation-placement="top-right"` on the UI surface. Place badges in a safe top-right spot that is visible, not clipped by `overflow: hidden`, and not causing labels or controls to wrap. Visual badges must contain plain digits such as `1`, `2`, and `3`, not circled numeral glyphs or nested badge content. Marker visual style must not change after click. Clicking a marker opens a small local body-only `annotation-dialog` popover beside that marker; clicking the same marker again closes it. The marker popover must not show a number badge, title/name, close button, horizontal scrollbar, full-screen/global modal, backdrop, or centered note dialog.
- A draggable annotation floating control with `data-draggable="true"` should show only `注释` in Chinese outputs or `Notes` in English outputs. Clicking it hides the floating control and opens an `annotation-list` panel from the right edge. The panel must fill viewport height, show annotations for the current page/state, restore the floating control when closed, and close when the user clicks outside the panel. The panel may include matching plain digit note numbers and titles.
...
- The default HTML annotation layer should expose one editable annotation configuration such as `annotationConfig.notes`; marker IDs, numbers, anchors/selectors, titles, and body text should be changed there first.
...
- Each UI marker and annotation-list note must share a stable annotation ID, for example `data-annotation-id="2"` on the UI badge and the matching `2` note in the current-state annotation panel. UI badges should also declare `data-annotation-placement="top-right"` unless a documented exception is needed. The marker popover itself should contain only the annotation body text; the full annotation list may show matching numbers and titles.
...
- Long annotation content should be concise enough for marker dialogs or expanded through the `annotation-list` overlay. Marker dialogs must not show the annotation number, title/name, close button, or horizontal scrollbar. Do not dump every page's notes into one long generic paragraph.
...
Generate a single self-contained `.html` file only when `portable_html` is selected or when `source_extract_html` is selected after a source-rendered preview or user-approved implementation exists. `prototype-<platform>.html` is the standard portable or source-extracted filename; `index.html` is allowed as an offline folder entry when the user explicitly asks for a portable/offline handoff. When repo-backed fidelity requires source rendering and no extracted HTML handoff is requested, generate the isolated preview route/story/demo/page/screen delta files instead and record them in `ui_delivery_trace.preview_files_changed`.
...
Portable HTML must record `portable_html_review_artifact` through metadata, comments, run-log fields, or PRD notes; source-extracted HTML must record its source-extraction metadata. Neither should add visible "example/demo/not production" copy to the product surface. The artifact must be structured and styled to a standard that UI and engineering can use directly as reference. A screenshot-only page is visual evidence, not a UI deliverable; portable HTML must contain real DOM text, controls, state changes, and annotation behavior.

For repo-backed UI-delivery-only work, self-contained HTML is acceptable as `source_extract_html` only after the target UI has been source-rendered or implemented under explicit user approval and the selected region is extracted with evidence. Compatibility self-contained HTML is acceptable only when the raw request asks for portable/standalone/HTML review without source implementation, explicitly asks to redesign/rebuild/from-scratch/stop reusing the original UI, or source rendering is concretely blocked. It may emulate inspected host components, token values, and local assets, but it cannot guarantee exact parity for icons, fonts, rendered component internals, browser-specific CSS, platform chrome, or runtime state. If frontend source exists, use a source-rendered preview mode after recording the mutation boundary and changed preview files. Production flows remain untouched unless production implementation is explicitly requested.
