...
- `需求详情` may contain repeated `prd-detail-media` blocks when a screenshot clarifies a state. In Markdown pipe tables, each source marker uses the exact inline syntax `[[prd-detail-media src="./assets/功能-状态.png" alt="功能-状态" copy="一、成功状态<br>1. 用户可见规则和反馈"]]`; never put a block-level `<div>` in a table cell. After Pandoc output, the renderer expands every marker into `<div class="prd-detail-media-block"><div class="prd-detail-media"><img src="./assets/功能-状态.png" alt="功能-状态" /></div><div class="prd-detail-copy">一、成功状态<br>1. 用户可见规则和反馈</div></div>`. The image is on the left and `copy` is its right-column requirement content: state-specific, user-visible behavior, boundary, or feedback. It must not be a filename, caption, or generic annotation such as “对应成功状态” or “见图”. Each block keeps the screenshot and its corresponding logic in the same cell; the outer requirement table remains one table and one `需求详情` cell. The renderer fixes the image column width across all blocks, preserves image proportions, and switches to stacked layout only on narrow screens or paged output. A standalone Markdown image, `<img>`, or `prd-detail-media-block` source HTML directly in `需求详情` is invalid.
...
Translate implementation evidence into the user-visible behavior it proves. Unless the user explicitly requests an engineering or visual-token specification, a PRD must not contain CSS variables or selectors, color values, gradients, stacking levels, exact dimensions, source field names, component names, code paths, or similar implementation parameters.
