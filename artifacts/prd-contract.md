...
When a field contains more than one concern, use `一、`、`二、`、`三、` to group the concerns, then use `1.`、`2.`、`3.` for the detailed rules under each group. In Markdown table cells, write every group heading and every numbered rule on its own explicit `<br>` line. Keep adjacent groups continuous with exactly one `<br>`; the blank-line gap rule applies only between separate media assets in a `图示` row. A `需求详情` cell with two or more rules must not compress them into one paragraph or use a flat list without a group heading. Apply this hierarchy to `需求入口`、`需求详情`、`设计与交互` and other fields when it improves reviewability; do not split exception handling, permissions, or recovery behavior into repetitive standalone rows.
...
For an implemented-feature PRD, every requirement that names a user-facing page, panel, dialog, node, toolbar, media state, or flow needs one visual-coverage decision. A named user-facing surface cannot be `not_required`: use a real figure when available; otherwise, after capture recovery, retain the `图示` row with a controlled placeholder and record every failed path in `run-log.yaml`:
