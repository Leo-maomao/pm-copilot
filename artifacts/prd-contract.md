...
- When `prd.html` is generated from `prd.md`, render it as a normal readable document with the fixed PM Copilot document shell: left table of contents, H1 excluded from the TOC, stable ASCII anchors, reading-position TOC sync, full-width content flow, complete readable left-aligned tables, rendered Mermaid diagrams through local assets, and inline images/placeholders at their relevant positions. Do not use `prototype-<platform>.html` naming for PRD document HTML.
