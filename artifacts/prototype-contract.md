...
Generate a single self-contained `.html` file only when standalone HTML is the selected artifact mode or when `source_extract_html` is selected after a source-rendered preview or user-approved implementation exists. `prototype-<platform>.html` is the default compatibility or source-extracted filename; `index.html` is allowed as an offline folder entry when the user explicitly asks for a portable/offline handoff. When repo-backed fidelity requires source rendering and no extracted HTML handoff is requested, generate the isolated preview route/story/demo/page/screen delta files instead and record them in `isolated_ui_prototype.preview_files_changed`.
...
Standalone HTML must record that it is a compatibility review artifact, source-extracted handoff, or documented fallback through metadata, comments, run-log fields, or PRD notes, but it should not add visible "example/demo/not production" copy to the product surface. It should be structured and styled to a standard that UI and engineering can use directly as reference. A screenshot-only page is visual evidence, not a prototype; compatibility HTML must contain real DOM text, controls, state changes, and annotation behavior.
