...
- For UI deliveries, record browser screenshot and visual diff validation under `visual_validation`. For compatibility HTML, `validate_prototype_visual.py` may be used; for source-backed preview/delta, run the host dev/preview/Storybook/simulator path and record equivalent evidence. If Playwright or browser tooling is unavailable, attempt or guide setup first; record `status: skipped` only with the exact setup failure, environment restriction, or user-declined reason.
