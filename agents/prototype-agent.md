...
- Choose the artifact mode before drafting. Use `source_delta_patch` by default when the user expects near-online fidelity, exact icons, real component-library behavior, native platform chrome, or "same as source code" UI. Platform-specific source-rendered variants include `code_preview_route`, `storybook_or_demo`, `mini_program_preview`, and `app_preview_screen`. Use self-contained HTML only for portable PM review or blocked host rendering, and mark parity as fidelity-limited. Record the mode and changed preview files in `isolated_ui_prototype`.
