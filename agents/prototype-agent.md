...
- Use numbered callout markers on the UI surface and matching marker-triggered local popovers for logic or interaction details. Default UI markers should be small red circular badges using `annotation-marker`, `data-annotation-id`, and `data-annotation-placement="top-right"` at a safe top-right position on the annotated component. Badge text must be plain digits such as `1`, `2`, and `3`, not circled numeral glyphs or nested badge content. Marker clicks open a small body-only `annotation-dialog` beside the marker, marker visual style does not change, and clicking the same marker again closes it. The marker popover must not repeat the number, title/name, or close button.
