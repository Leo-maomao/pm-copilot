...
- Produce the selected UI deliverable: a source-rendered preview/delta for repo-backed source-present UI, or a portable standalone HTML artifact when that mode is selected.
...
- When the user explicitly asks to implement the feature in the current repository first, complete or adjust the approved host-source implementation before extraction, then treat that running implementation as the source of truth for the 1:1 handoff and record the changed source files separately from generated handoff files.
...
- When the user supplies a screenshot, mockup, concept image, cropped UI image, or asks for image-to-UI / screenshot reconstruction / "图片还原", activate the Image Reference Reconstruction Mode inside `skills/multi-platform-ui-delivery/SKILL.md`: record reference dimensions, inventory every visible UI element, match the reference viewport first, use screenshot comparison when available, and keep iterating until remaining differences are minor and documented.
- Load and apply `skills/multi-platform-ui-delivery/SKILL.md`, `artifacts/ui-delivery-contract.md`, and `tools/ui-delivery-tooling.md` before writing UI deliverables.
...
- Choose the artifact mode before drafting. Use `source_delta_patch` by default whenever host frontend source and a preview surface exist, not only when the user asks for exact fidelity. Platform-specific source-rendered variants include `code_preview_route`, `storybook_or_demo`, `mini_program_preview`, and `app_preview_screen`. Use `source_extract_html` when the PM needs a standalone HTML handoff after the target UI has already been rendered in the current host project; extraction is source-derived, not a hand-written fallback. The source may be an isolated preview delta by default or production-oriented implementation files when the user explicitly asks for current-repo implementation first. Use portable HTML mode only when the user's raw request explicitly asks for a portable/standalone/HTML artifact without source implementation, explicitly asks to redesign/rebuild/from-scratch/stop reusing the original UI, or source rendering was attempted and blocked by concrete command, browser, simulator, dependency, or preview-surface evidence. "Only generate a prototype" means review scope only; it does not authorize standalone HTML or greenfield UI. Production read-only policy is not a blocker because isolated preview files are allowed; user-approved implementation work must be recorded as such. Mark fallback parity as fidelity-limited and record the mode and changed preview or implementation files in `ui_delivery_trace`.
- For `source_extract_html`, run or document the extraction pass after source preview or implementation validation: target preview URL/file, extraction selector(s), source-region screenshot, extracted HTML path, style capture method, asset handling, editable annotation configuration, validation report, source-change scope, and limitations. If the selector misses the intended component, if the source preview cannot be opened, or if extracted styles/assets are materially incomplete, return `degraded` and keep the source-backed preview as the authoritative reference.
- Do not hand off only a localhost URL for source-backed previews. Record the preview command, route/screen/story, changed preview/delta files, and validation evidence. Generate direct HTML only when the selected artifact mode allows portable HTML HTML; otherwise explain that exact parity depends on rendering host source.
...
- Use numbered callout markers on the UI surface and matching marker-triggered local popovers for logic or interaction details. Generate the default markers from an editable annotation configuration so the PM can add, remove, reorder, or edit notes after handoff. Default UI markers should be small red circular badges using `annotation-marker`, `data-annotation-id`, and `data-annotation-placement="top-right"` at a safe top-right position on the annotated component. Badge text must be plain digits such as `1`, `2`, and `3`, not circled numeral glyphs or nested badge content. Marker clicks open a small body-only `annotation-dialog` beside the marker, marker visual style does not change, and clicking the same marker again closes it. The marker popover must not repeat the number, title/name, or close button.
- Use an annotation floating control with only `注释` in Chinese outputs or `Notes` in English outputs. It uses `data-draggable="true"`, hides when clicked, opens a right-edge full-height current-state annotation panel, and reappears when that panel closes. The side panel may include matching plain digit note numbers and titles, and it also closes when the user clicks outside the panel.
- Keep reviewer-only page/state switching controls fixed, collapsed, marked with `data-reviewer-only="true"`, and outside the product layout when reviewers need them. Do not use a prominent row of state pills as the primary interaction model.
...
- Record `portable_html` as a portable review artifact in metadata, comments, run log, and PRD notes. Do not place visible "example/demo/not production" labels in the product UI unless the product requirement itself needs visible draft status for unreviewed content. Source-backed preview/delta files may be implementation candidates only when the user explicitly asked for implementation-oriented work and the host mutation policy records that boundary.
...
- Use `tools/ui-delivery-tooling.md` and `tools/validation-tooling.md` for UI delivery verification expectations.
...
- UI deliverable: source-rendered preview/delta files or portable HTML artifact
- Source-extracted HTML handoff when selected: `prototype-<platform>.html` or offline `index.html` derived from a running host-source preview or user-approved host implementation plus source preview and extracted HTML evidence
...
- Source-backed preview handoff includes the preview command, route/screen/story, and changed files; portable HTML handoff includes the generated HTML path.
...
- Critical states are driven by real UI controls or mocked data transitions, not only by reviewer state tabs.
...
- If a standalone HTML file is delivered from a renderable repo-backed UI, it must either be `source_extract_html` with source preview evidence and extraction metadata, or an explicitly limited portable artifact justified by the raw request or a concrete blocker.
