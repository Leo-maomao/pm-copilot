...
- Build the requirement list before drafting details. Every list row must identify its matching detail number, target user, user scenario or trigger, user problem/value, priority, and source status. Use the detail number as the only requirement identifier.
...
- Clean input before drafting: retain user-confirmed facts, directly observed behavior, and source-backed research; discard template instructions, process narration, implementation evidence, duplicate explanation, and vague filler. Mark required decision fields as `待确认` with a reason only when the unresolved rule affects product behavior; do not use model intuition to invent users, limits, permissions, states, or existing behavior. Remove conditional fields and subsections that do not apply instead of emitting filler.
