...
- Make pre-validation deterministic traces internally consistent: pending validation, non-final review/action states, and controller-owned finalization now use matching loop and readiness semantics.
...
- Generalize in-place revision trace materialization to derive changed requirement scope from the confirmed request and staged PRD diff, filter retired IDs, and emit one trace coverage decision per revised requirement.
...
- Record structured artifact and validation deltas for deterministic revision trace handoffs, including HTML and fixed-figure evidence.
...
- Keep deterministic trace artifacts stage-reviewable with a passed preflight contract, then let controller finalization record aggregate delivery validation.

## [6.2.80] - 2026-09-03

- Finalize deterministic traces before the first validation gate can short-circuit on their intentional pending state.

## [6.2.79] - 2026-09-03

- Make deterministic revision traces truthful before final validation and atomically record actual validator results before declaring completion.

## [6.2.78] - 2026-09-03

- Separate the interactive delivery aggregate timeout from the per-Agent execution timeout and persist `recovery_required` when the aggregate budget is exhausted.

## [6.2.77] - 2026-09-03

### Fixed

- Refuse duplicate `--confirm` controllers while an existing delivery PID is alive, preventing repeated staging resets and endless stage loops.

## [6.2.76] - 2026-09-03

### Fixed

- Remove remaining historical scope and asset-count wording from deterministic revision traces so trace review matches the confirmed 5.1 boundary.

## [6.2.75] - 2026-09-03

### Fixed

- Align deterministic revision traces with validator contracts for unique visual coverage, tool evidence, successful loop termination, and empty memory candidates.

## [6.2.74] - 2026-09-03

### Fixed

- Recover stale `delivery/running` states through a controller PID lease when the parent process exits before it can checkpoint termination.

## [6.2.73] - 2026-09-03

### Fixed

- Catch controller `SIGTERM` and `SIGINT` during delivery so interrupted runs checkpoint a terminal failure instead of remaining `delivery/running`.
...
- Retry stage Agents after bounded no-progress or timeout failures, including review calls that fail to persist their required JSON evidence.

## [6.2.68] - 2026-09-03

### Fixed

- Clarify that the PRD Agent writes only `prd.md`; the controller owns downstream `prd.html` rendering and validation, preventing a false artifact-scope refusal during fallback execution.
...
- Preserve the failed Seawork call's device-advertised model catalog for bounded fallback when a transient daemon probe returns no live candidates.

## [6.2.65] - 2026-09-03

### Fixed

- Treat a terminal Seawork Agent `error` as a bounded runtime failure and enter the device-discovered model fallback chain instead of returning an unattributable error immediately.
...
- Rewrite the deterministic in-place revision trace's task, strategy, checkpoint, readiness, and next-action sections from the confirmed 5.1 revision scope, preventing stale multi-requirement and three-figure history from reaching trace review.
...
- Require a changed artifact in the controller's isolated stage workspace before promotion; reject idle Agents that leave an old target unchanged, restore delivery-workspace files written through the wrong path, and retain stream-disconnected writes as unpromoted evidence.
...
- After a Seawork polling deadline, refresh its control-plane record once before stopping it; accept an attributable Agent when its promised artifact changed during this call and the refresh reports a successful terminal state such as `idle`.
...
- Align regression consistency coverage with the independent requirement-detail group numbering contract.

## [6.2.42] - 2026-08-31

### Fixed

- Reset Chinese requirement-detail group and rule numbering for each independently presented content or media block, with validator coverage for invalid continuation numbering.
...
- Force a fresh PRD iframe request after index refresh so the selected document does not remain blank when its URL is unchanged.
