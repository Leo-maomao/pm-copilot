...
- Inspect current mini-program UI evidence before UI delivery.
...
- UI deliverable matches the current mini-program visual style: status bar, capsule/header, tab bar, colors, spacing, typography, icon style, card radius, shadows, and layout density.
- UI deliverable does not look like a newly invented product shell when existing UI evidence is available.
...
| Run ID |  |
| Status | Pending |
| Notes | Regression case for mini-program UI delivery style fidelity and numbered implementation annotations. |
