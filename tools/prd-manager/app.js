(() => {
  ...
  function renderTree() {
    ...
    tree.querySelectorAll('.document-row').forEach((row) => row.onclick = (event) => { if (event.target.closest('.reveal-document, .copy-document')) return; selectDocument(row.querySelector('.document-item').dataset.id); });
    tree.querySelectorAll('.document-item').forEach((button) => button.onclick = () => selectDocument(button.dataset.id));
    tree.querySelectorAll('.copy-document').forEach((button) => button.onclick = async (event) => { event.stopPropagation(); try { const response = await fetch(`/api/document/${encodeURIComponent(button.dataset.id)}/copy`, { method: 'POST' }); if (!response.ok) throw new Error('无法复制目录'); button.classList.add('is-copied'); button.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m5 12 4 4L19 6"/></svg>'; button.setAttribute('aria-label', '已复制目录'); button.title = '已复制目录'; window.setTimeout(() => { button.classList.remove('is-copied'); button.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="8" y="8" width="11" height="11" rx="1.5"/><path d="M5 16V5.5A1.5 1.5 0 0 1 6.5 4H17"/></svg>'; button.setAttribute('aria-label', `复制 ${button.closest('.document-row').querySelector('.document-title').textContent} 所在目录`); button.title = '复制所在目录'; }, 1200); } catch (error) { button.title = '复制失败'; window.setTimeout(() => { button.title = '复制所在目录'; }, 1200); } });
