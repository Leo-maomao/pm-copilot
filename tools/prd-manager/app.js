(() => {
  ...
  function selectDocument(id, query = '', restoreScroll = undefined) {
    ...
    empty.hidden = true; viewer.hidden = false; viewer.classList.add('is-loading'); viewer.src = `/document/${encodeURIComponent(id)}/?v=${Date.now()}${targetHash}`;
    viewer.onload = () => { const reveal = () => viewer.classList.remove('is-loading'); if (targetScroll && !query) { const restore = () => { const frameDocument = viewer.contentDocument; const frameWindow = viewer.contentWindow; if (!frameDocument || !frameWindow) return; frameDocument.documentElement.style.scrollBehavior = 'auto'; if (frameDocument.body) frameDocument.body.style.scrollBehavior = 'auto'; const scrollingElement = frameDocument.scrollingElement || frameDocument.documentElement || frameDocument.body; if (scrollingElement) { scrollingElement.scrollLeft = targetScroll.left; scrollingElement.scrollTop = targetScroll.top; } frameWindow.scrollTo({ left: targetScroll.left, top: targetScroll.top, behavior: 'auto' }); }; restore(); requestAnimationFrame(() => { restore(); setTimeout(() => { restore(); reveal(); }, 80); }); setTimeout(reveal, 500); } else { if (query) highlight(query); reveal(); } viewer.contentWindow?.addEventListener('scroll', () => rememberViewerScroll(id), { passive: true }); }; document.querySelector('.sidebar').classList.remove('open');
  }
  ...
  function openSearch() { dialog.showModal(); input.focus(); input.select(); search(); refresh().then(() => search()).catch(() => {}); }
  function loadIndex(payload, restoreScroll = undefined) { state.index = payload; $('index-status').textContent = `${payload.count} 份`; renderTree(); const choice = allDocuments().some((item) => item.id === state.selectedId) ? state.selectedId : allDocuments()[0]?.id; if (choice) selectDocument(choice, '', restoreScroll); else if (payload.scanning) { empty.querySelector('h1').textContent = '正在整理 PRD'; empty.querySelector('p').textContent = '首次扫描正在后台进行，完成后可点击刷新。'; } else { empty.querySelector('h1').textContent = '尚未找到 PRD'; empty.querySelector('p').textContent = '请确认项目中存在 pm-copilot-outputs/<run-id>/prd.html。'; } }
  async function refresh() { const button = $('refresh-button'); const currentId = state.selectedId; const restoreScroll = currentId ? readViewerScroll() : null; if (currentId && restoreScroll) localStorage.setItem(scrollStorageKey(currentId), JSON.stringify(restoreScroll)); button.disabled = true; button.classList.add('is-refreshing'); try { const response = await fetch('/api/refresh', { method:'POST' }); if (!response.ok) throw new Error('刷新失败'); loadIndex(await response.json(), currentId === state.selectedId ? restoreScroll : null); } catch (error) { /* Keep the prior count when refresh cannot complete. */ } finally { button.disabled = false; button.classList.remove('is-refreshing'); } }
  ...
  window.addEventListener('pagehide', () => { if (state.selectedId) rememberViewerScroll(state.selectedId); });
