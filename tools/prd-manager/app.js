(() => {
  ...
  function selectDocument(id, query = '', restoreScroll = null) {
    ...
    viewer.onload = () => { if (restoreScroll && !query) { const restore = () => viewer.contentWindow?.scrollTo(restoreScroll.left, restoreScroll.top); restore(); requestAnimationFrame(() => { restore(); setTimeout(restore, 80); }); } if (query) highlight(query); }; document.querySelector('.sidebar').classList.remove('open');
  }
  ...
  async function refresh() { const button = $('refresh-button'); const currentId = state.selectedId; const restoreScroll = currentId ? readViewerScroll() : null; button.disabled = true; button.classList.add('is-refreshing'); try { const response = await fetch('/api/refresh', { method:'POST' }); if (!response.ok) throw new Error('刷新失败'); loadIndex(await response.json(), currentId === state.selectedId ? restoreScroll : null); } catch (error) { /* Keep the prior count when refresh cannot complete. */ } finally { button.disabled = false; button.classList.remove('is-refreshing'); } }
