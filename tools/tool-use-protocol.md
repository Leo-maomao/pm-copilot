...
For portable HTML UI review artifacts, run `python3 scripts/validate_prototype_visual.py outputs/<run-id>`. With no `--prototype` argument, the visual validator checks every supported portable HTML file in the run folder and writes one aggregate `visual-report.json`. Existing host previews may be inspected with `validate_ui_preview.py` and recorded as read-only evidence under `visual_validation`. To capture an existing UI region, run `python3 scripts/extract_ui_region.py --target <preview-url-or-file> --selector '<css-selector>' --output outputs/<run-id>/prototype-<platform>.html --run-folder outputs/<run-id>`, then state that the extract documents existing behavior rather than a proposed feature. Browser automation defaults to Playwright-managed cached browsers; system browser channels are explicit overrides only. If validation cannot run because the dependency or browser is unavailable, first run `python3 scripts/setup_visual_validation.py` or guide the user through the same setup. Record visual validation as `skipped` only if setup fails, browser launch is forbidden, or the user declines; do not let the absence of visual QA look like a passed browser review.
