...
The visual validator checks every supported prototype file in the run folder unless `--prototype <file>` is used to isolate one platform. If an auto-detected system browser fails to launch, it should fall back to Playwright's default/bundled Chromium path and attempt setup before reporting a browser limitation.
